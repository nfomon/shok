  ...
  void RemoveListener(const T1& t1, const T2& t2) {
    {
      t1_mod_iter i = m_t1_map.find(t1);
      if (m_t1_map.end() == i) {
        throw FWError("Cannot remove listener; no listeners for this item");
      }
      listener_set& listeners = i->second;
      listener_iter j = listeners.find(t2);
      if (listeners.end() == j) {
        throw FWError("Cannot remove listener; output item not listening to this input item");
      }
      listeners.erase(t2);
    } {
      t2_mod_iter i = m_t2_map.find(t2);
      if (m_t2_map.end() == i) {
        throw FWError("Cannot remove listener; somehow not listening to this item");
      }
      t1_set& t1s = i->second;
      t1_set_iter j = t1s.find(t1);
      if (t1s.end() == j) {
        throw FWError("Cannot remove listener; input item not being listened by this output item");
      }
      t1s.erase(t1);
    }
  }
